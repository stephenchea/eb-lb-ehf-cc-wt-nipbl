...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/share/crsp/lab/alcalof/schea2/20220414-seurat/flox-integr-res-0.0120/flox-*-cluster-iters /Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120
```
...
for (i in seq(from = 0, to = max.res, by = 0.001)) {
  file.name <- paste0("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-", n, "-cluster-iters/cluster-per-cell-flox-", n, "-res-", sprintf(fmt = "%.3f", i), ".Robj")
  load(file.name)
  cluster.cell.list[[sprintf(fmt = "%.3f", i)]] <- as.numeric(cluster.cluster)
}
