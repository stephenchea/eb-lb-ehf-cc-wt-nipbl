...
```{r}
load("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/fin-END-projected-flox-END-3/cluster-per-cell-fin-END-3.Robj")
```

Load Flox cells.
```{r}
load("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/fin-END-projected-flox-END-3/fin-END-projected-flox-END-3.Robj")
```

Set cell identities. 
```{r}
fin.germ$cluster.germ.flox <- cluster.germ.flox
Idents(fin.germ) <- cluster.germ.flox
```

Load DEGs.
```{r}
load("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/fin-END-projected-flox-END-3/projected-diff-genes-fin-END-projected-flox-END-3.Robj")
```
...
expr.umap <- as.data.frame(t(as.data.frame(fin.germ@assays$SCT_END@data[top.genes,])))

#convert column names
colnames(expr.umap) <- str_replace(top.genes, pattern = "-", replacement = "_")

#add umap embedding and cluster identities
expr.umap <- cbind(expr.umap, fin.germ@reductions$ref.umap@cell.embeddings)
...
dir.create("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/fin-END-projected-flox-END-3/feat-plot-fin-END-projected-flox-END-3")
```

```{r}
loop.count = 0
for (i in colnames(expr.umap)[c(1:138, 140:156)]) {
...
feat.plot <- ggplot(data = sample(expr.umap), mapping = aes(x = refUMAP_1, y = refUMAP_2)) +
    ...
    labs(title = str_replace(i, pattern = "_", replacement = "-"), subtitle = "Endoderm, Nipbl FIN/+", x = "UMAP 1", y = "UMAP 2", color = "Norm\nUMI") +
...
filename.png <- paste0("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/fin-END-projected-flox-END-3/feat-plot-fin-END-projected-flox-END-3/", top.clusters[i], "-", string.loop.count, "-", str_replace(i, pattern = "_", replacement = "-"), "-feat-plot-fin-END-projected-flox-END-3.png")
