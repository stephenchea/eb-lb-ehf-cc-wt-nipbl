...
for (i in c("0.0022", "0.0065", "0.0092", "0.0151", "0.0259", "0.0300", "0.0554", "0.0431", "0.0510", "0.0624")) {
  file.name <- paste0("/Volumes/all_ssd/20220414-seurat/fin-integr-cluster-iters/cluster-per-cell-fin-integr-res-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i]] <- as.numeric(fin.integr.active.ident)
}
...
res <- c("0.0022", "0.0065", "0.0092", "0.0151", "0.0259", "0.030", "0.0554", "0.0431", "0.0510", "0.0624")
...
tidy.node$resolution <- factor(tidy.node$resolution, levels = c("0.0022", "0.0065", "0.0092", "0.0151", "0.0259", "0.030", "0.0554", "0.0431", "0.0510", "0.0624"))
...
ggsave(plot = cluster.tree, filename = "/Volumes/all_ssd/20220414-seurat/cluster-tree-fin-integr.png", device = "png", units = "in", width = 4.33, height = 6.75)
```
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = "Nipbl FIN/+", x = "Resolution", y = "Total Shannon Entropy", color = "") +
  theme_classic(base_size = 7) + 
...
ggsave(plot = shannon.plot, filename = "/Volumes/all_ssd/20220414-seurat/summary-shannon-fin-integr.png", device = "png", units = "in", width = 3.25, height = 1.28)
```
