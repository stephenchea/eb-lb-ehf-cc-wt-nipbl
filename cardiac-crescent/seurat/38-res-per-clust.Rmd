...
for (i in seq(from = 0, to = max.res, by = 0.0001)) {
  file.name <- paste0("/Volumes/all_ssd/20220414-seurat/fin-integr-cluster-iters/cluster-per-cell-fin-integr-res-", sprintf(fmt = "%.4f", i), ".Robj")
  load(file.name)
  cluster.cell.list[[sprintf(fmt = "%.4f", i)]] <- as.numeric(fin.integr.active.ident)
}
