...
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-END-3/cluster-per-cell-flox-END-3.Robj")
```

Load Flox cells.
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-END-umap.Robj")
```
...
Load DEGs.
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-END-3/diff-genes-flox-END-3.Robj")
```
...
expr.umap <- as.data.frame(t(as.data.frame(flox.germ@assays$SCT_END@data[top.genes,])))
...
dir.create("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-END-3/feat-plot-flox-END-3")
```

```{r}
loop.count = 0
for (i in colnames(expr.umap)[c(1:142, 144:156)]) {
...
feat.plot <- ggplot(data = sample(expr.umap), mapping = aes(x = UMAP_1, y = UMAP_2)) +
    ...
    labs(title = str_replace(i, pattern = "_", replacement = "-"), subtitle = "Endoderm, Nipbl Flox/+", x = "UMAP 1", y = "UMAP 2", color = "Norm\nUMI") +
...
filename.png <- paste0("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-END-3/feat-plot-flox-END-3/", top.clusters[i], "-", string.loop.count, "-", str_replace(i, pattern = "_", replacement = "-"), "-feat-plot-flox-END-3.png")
