...
res <- as.character(c("0.0382", "0.0236", "0.0120", "0.0103", "0.0053"))

for (i in 1:length(res)) {

filename.robj <- paste0("/share/crsp/lab/alcalof/schea2/20220414-seurat/flox-integr-cluster-iters/cluster-per-cell-flox-integr-res-", res[i], ".Robj")
...
path.direct <- paste0("/share/crsp/lab/alcalof/schea2/20220414-seurat/flox-integr-res-", res[i])
...
diff.genes <- FindAllMarkers(flox.integr, assay = "SCT_GENOTYPE", logfc.threshold = 0, test.use = "wilcox", slot = "data", min.pct = 0, verbose = TRUE, only.pos = TRUE, max.cells.per.ident = dim(flox.integr)[2]/length(levels(flox.integr$cluster.whole)), return.thresh = 0.05)

filename.robj <- paste0("diff-genes-flox-integr-res-", res[i], ".Robj")
save(diff.genes, file = filename.robj)

filename.csv <- paste0("diff-genes-flox-integr-res-", res[i], ".csv")
write.csv(x = diff.genes, file = filename.csv)

#top.markers <- diff.genes %>% group_by(cluster) %>% slice_max(myAUC, n = 177/length(levels(flox.integr$cluster.whole)))
...
#filename.png <- paste0("heatmap-diff-genes-flox-integr-res-", res[i], ".png")
