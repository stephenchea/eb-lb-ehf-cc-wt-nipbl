...
for (i in c("3.1")) {
    ...
    for (t in seq(from = 0, to = 0.5, by = 0.0001)) {
    ...
    filename.robj <- paste0("cluster-per-cell-flox-", i, "-res-", sprintf(fmt = "%.4f", t), ".Robj")
    ...
    filename.png <- paste0("umap-by-cluster-flox-", i, "-res-", sprintf(fmt = "%.4f", t), ".png")
