...
ident.clust <- c("3.1")
optim.clust <- list("3.1" = c("0.0144", "0.0387", "0.0704"))

for (i in 1:length(ident.clust)) {
    
    for (t in 1:length(optim.clust[[i]])) {
    
    setwd("/share/crsp/lab/alcalof/schea2/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025")
...
path.direct <- paste0("/share/crsp/lab/alcalof/schea2/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025/", name.direct)
setwd(path.direct)
...
subclust.umap <- data.frame(flox.subclust@reductions$umap@cell.embeddings, flox.subclust$stage, flox.subclust$label.embryo, flox.subclust$cluster.subclust)
...
flox.subclust <- PrepSCTFindMarkers(flox.subclust, assay = name.assay, verbose = TRUE)

diff.genes <- FindAllMarkers(flox.subclust, assay = name.assay, logfc.threshold = 0, test.use = "wilcox", slot = "data", min.pct = 0, verbose = TRUE, only.pos = TRUE, max.cells.per.ident = dim(flox.subclust)[2]/length(levels(flox.subclust$cluster.cluster)), return.thresh = 0.05)
