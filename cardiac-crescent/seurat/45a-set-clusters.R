...
path.direct <- paste0("/share/crsp/lab/alcalof/schea2/20220414-seurat/fin-integr-res-0.0151/", name.direct)
setwd(path.direct)
...
fin.clust <- PrepSCTFindMarkers(fin.clust, assay = name.assay, verbose = TRUE)

diff.genes <- FindAllMarkers(fin.clust, assay = name.assay, logfc.threshold = 0, test.use = "wilcox", slot = "data", min.pct = 0, verbose = TRUE, only.pos = TRUE, max.cells.per.ident = dim(fin.clust)[2]/length(levels(fin.clust$cluster.cluster)), return.thresh = 0.05)
...
#top.markers <- diff.genes %>% group_by(cluster) %>% slice_max(myAUC, n = 79/length(levels(fin.clust$cluster.cluster)))
