...
for (i in c("0.0017", "0.0062", "0.0097", "0.0158", "0.0231", "0.0264", "0.0302", "0.0384", "0.0328", "0.0625")) {
  file.name <- paste0("/Volumes/all_ssd/20220414-seurat/all-aggr-cluster-iters/cluster-per-cell-all-aggr-res-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i]] <- as.numeric(all.aggr.active.ident)
}
...
res <- c("0.0017", "0.0062", "0.0097", "0.0158", "0.0231", "0.0264", "0.0302", "0.0384", "0.0328", "0.0625")
...
tidy.node$resolution <- factor(tidy.node$resolution, levels = c("0.0017", "0.0062", "0.0097", "0.0158", "0.0231", "0.0264", "0.0302", "0.0384", "0.0328", "0.0625"))
...
cluster.tree <- ggraph(graph = cluster.tree.layout, layout = "tree") +
 ...
 scale_edge_alpha_continuous(range = c(0.2, 1)) +
  ...
  annotate("rect", xmin=-Inf, xmax=Inf, ymin=3.5, ymax=4.5, color="red3", alpha = 0)

cluster.tree

ggsave(plot = cluster.tree, filename = "/Volumes/all_ssd/20220414-seurat/cluster-tree-all-aggr.png", device = "png", units = "in", width = 4.33, height = 6.75)
```
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(x = "Resolution", y = "Total Shannon Entropy", color = "") +
  theme_classic(base_size = 7) + 
  theme(plot.title = element_text(hjust = 0.5), legend.position = "right") +
  annotate("rect", xmin=5.5, xmax=6.5, ymin=-Inf, ymax=Inf, color="red3", alpha = 0)

shannon.plot

ggsave(plot = shannon.plot, filename = "/Volumes/all_ssd/20220414-seurat/summary-shannon-all-aggr.png", device = "png", units = "in", width = 3.25, height = 1.28)
```
