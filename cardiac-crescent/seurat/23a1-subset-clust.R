...
for (i in c("3.1")) {
    ...
    load("cluster-per-cell-flox-3-res-0.025.Robj")
    ...
    flox.clust[["SCT_3"]] <- NULL
    flox.clust[["INT_3"]] <- NULL
    ...
    flox.subclust <- IntegrateData(anchorset = anchors.flox, normalization.method = "SCT", features.to.integrate = feat.integr, new.assay.name = int.assay.name, k.weight = min(c(100, min.cells*0.82)), dims = 1:min(c(30, min.cells*0.95)))
    ...
    direct.path <- paste0("/share/crsp/lab/alcalof/schea2/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025/", direct.name)
    setwd(direct.path)
