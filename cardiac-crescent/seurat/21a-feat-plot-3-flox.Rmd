...
Load Flox cells.
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-3-neighbors.Robj")
```

Load markers.
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025/diff-genes-flox-3-res-0.025.Robj")
head(diff.genes)
```
...
expr.umap <- as.data.frame(t(as.data.frame(flox.clust@assays$SCT_3@data[top.genes,])))

#convert column names
colnames(expr.umap) <- str_replace(top.genes, pattern = "-", replacement = "_")

#add umap embedding and cluster identities
expr.umap <- cbind(expr.umap, flox.clust@reductions$umap@cell.embeddings)
...
dir.create("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025/feat-plot-flox-3-res-0.025")
```
...
for (i in colnames(expr.umap)[c(1:114, 116:149, 151:158)]) {
...
feat.plot <- ggplot(data = sample(expr.umap), mapping = aes(x = UMAP_1, y = UMAP_2)) +
    ...
    labs(title = str_replace(i, pattern = "_", replacement = "-"), subtitle = "Cluster 3, Nipbl Flox/+", x = "UMAP 1", y = "UMAP 2", color = "Norm\nUMI") +
...
filename.png <- paste0("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025/feat-plot-flox-3-res-0.025/", top.clusters[i], "-", string.loop.count, "-", str_replace(i, pattern = "_", replacement = "-"), "-feat-plot-flox-3-res-0.025.png")
