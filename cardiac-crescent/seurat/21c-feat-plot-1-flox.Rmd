...
Load Flox cells.
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-1-neighbors.Robj")
```

Load markers.
```{r}
load("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-1-res-0.074/diff-genes-flox-1-res-0.074.Robj")
head(diff.genes)
```
...
expr.umap <- as.data.frame(t(as.data.frame(flox.clust@assays$SCT_1@data[top.genes,])))
...
dir.create("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-1-res-0.074/feat-plot-flox-1-res-0.074")
```
...
for (i in colnames(expr.umap)[c(1:152, 154:155)]) {
...
feat.plot <- ggplot(data = sample(expr.umap), mapping = aes(x = UMAP_1, y = UMAP_2)) +
    ...
    labs(title = str_replace(i, pattern = "_", replacement = "-"), subtitle = "Cluster 1, Nipbl Flox/+", x = "UMAP 1", y = "UMAP 2", color = "Norm\nUMI") +
...
filename.png <- paste0("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-1-res-0.074/feat-plot-flox-1-res-0.074/", top.clusters[i], "-", string.loop.count, "-", str_replace(i, pattern = "_", replacement = "-"), "-feat-plot-flox-1-res-0.074.png")
