...
Idents(flox.integr) <- umap.whole$cluster.whole
...
#set identity to germ layer
Idents(flox.stage) <- flox.stage$cluster.whole

#print("preparing to find diff genes")
#flox.stage <- PrepSCTFindMarkers(flox.stage, assay = "SCT_GENOTYPE", verbose = TRUE)

print("finding diff genes")
diff.genes <- FindAllMarkers(flox.stage, assay = "SCT_GENOTYPE", logfc.threshold = 0, test.use = "wilcox", slot = "data", min.pct = 0, verbose = TRUE, only.pos = TRUE, max.cells.per.ident = dim(flox.stage)[2]/length(levels(flox.stage$cluster.whole)), return.thresh = 0.05)

save(diff.genes, file = "diff-genes-between-germ-layers-LB-flox-integr-res-0.0120-26.Robj")

write.csv(x = diff.genes, file = "diff-genes-LB-flox-integr-res-0.0120-26.csv")
...
write.csv(x = diff.genes, file = "diff-genes-CC-flox-integr-res-0.0120-26.csv")
