...
Load FIN cells.
```{r}
load("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/fin-integr-projected-flox-integr-res-0.0120.Robj")
```

Load markers.
```{r}
load("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/projected-diff-genes-fin-integr-projected-flox-integr-res-0.0120.Robj")
head(diff.genes)
```
...
expr.umap <- as.data.frame(t(as.data.frame(fin.integr@assays$SCT_GENOTYPE@data[top.markers,])))

#convert column names
colnames(expr.umap) <- str_replace(top.markers, pattern = "-", replacement = "_")

#add umap embedding and cluster identities
expr.umap <- cbind(expr.umap, fin.integr@reductions$ref.umap@cell.embeddings)
...
dir.create("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/feat-plot-diff-genes-fin-integr-projected-flox-integr-res-0.0120")
```
...
feat.plot <- ggplot(data = sample(expr.umap), mapping = aes(x = refUMAP_1, y = refUMAP_2)) +
    ...
    labs(title = str_replace(i, pattern = "_", replacement = "-"), subtitle = "Whole Embryo, Nipbl FIN/+", x = "UMAP 1", y = "UMAP 2", color = "Norm\nUMI") +
...
filename.png <- paste0("/Volumes/all_ssd/20220414-seurat/fin-integr-projected-flox-integr-res-0.0120/feat-plot-diff-genes-fin-integr-projected-flox-integr-res-0.0120/", top.clusters[i], "-", string.loop.count, "-", str_replace(i, pattern = "_", replacement = "-"), "-feat-plot-diff-genes-fin-integr-projected-flox-integr-res-0.0120.png")
