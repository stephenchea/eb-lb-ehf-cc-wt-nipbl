...
for (i in seq(from = 0, to = max.res, by = 0.0001)) {
  file.name <- paste0("/Volumes/all_ssd/20220414-seurat/flox-integr-res-0.0120/flox-3-res-0.025/flox-", n, "-cluster-iters/cluster-per-cell-flox-", n, "-res-", sprintf(fmt = "%.4f", i), ".Robj")
  load(file.name)
  cluster.cell.list[[sprintf(fmt = "%.4f", i)]] <- as.numeric(cluster.subclust)
}
