...
Download DEGs.
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:'/Volumes/all_ssd/20220414-seurat/flox-integr-res-*' /Volumes/all_ssd/20220414-seurat
```
...
filename.robj <- paste0(path.direct, "cluster-per-cell-flox-integr-res-", res[i], ".Robj")
load(file = filename.robj)

flox.integr@active.ident <- cluster.whole
flox.integr$cluster.whole <- cluster.whole

filename.robj <- paste0(path.direct, "umap-by-cluster-flox-integr-res-", res[i], ".Robj")
load(file = filename.robj)
...
filename.png <- paste0(path.direct, "umap-by-cluster-flox-integr-res-", res[i], ".png")
ggsave(plot = cluster.umap, filename = filename.png, device = "png", width = 3.5, height = 2.25, units = "in")

filename.robj <- paste0(path.direct, "diff-genes-flox-integr-res-", res[i], ".Robj")
load(file = filename.robj)

top.genes <- diff.genes %>% group_by(cluster) %>% slice_head(n = 79/length(levels(flox.integr$cluster.whole)))

cell.names <- rownames(flox.integr@meta.data)

heatmap <- DoHeatmap(flox.integr, cells = sample(cell.names, round(length(cell.names)*0.5), replace = FALSE), assay = "SCT_GENOTYPE", slot = "scale.data", features = as.vector(top.genes$gene), group.by = "ident", group.bar = TRUE, label = TRUE, size = 2.3, angle = 0, hjust = 0.5, draw.lines = TRUE) +
...
filename.png <- paste0(path.direct, "heatmap-diff-genes-flox-integr-res-", res[i], ".png")
