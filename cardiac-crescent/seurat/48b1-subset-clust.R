...
for (i in c("2.3")) {
    ...
    for (a in 1:length(embryo.fin)) {
        embryo.fin[[a]] <- RunPCA(embryo.fin[[a]], assay = "SCT_EMBRYO", features = feat.integr, npcs = min(c(50, min.cells*0.82)), verbose = TRUE)
    }

    anchors.fin <- FindIntegrationAnchors(object.list = embryo.fin, normalization.method = "SCT", anchor.features = feat.integr, reduction = "rpca", k.score = min(c(30, min.cells*0.95)), k.filter = min(c(200, min.cells*0.95)), dims = 1:min(c(30, min.cells*0.82)))
