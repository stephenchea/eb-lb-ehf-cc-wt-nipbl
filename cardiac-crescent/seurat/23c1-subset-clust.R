...
for (i in c("1.3")) {
    ...
    load("cluster-per-cell-flox-1-res-0.074.Robj")
    ...
    flox.clust[["SCT_1"]] <- NULL
    flox.clust[["INT_1"]] <- NULL
    ...
    flox.subclust <- IntegrateData(anchorset = anchors.flox, normalization.method = "SCT", features.to.integrate = feat.integr, new.assay.name = int.assay.name, k.weight = min(c(100, min.cells*0.68)), dims = 1:min(c(30, min.cells*0.95)))
