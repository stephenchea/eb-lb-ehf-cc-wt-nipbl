...
load("fin-M1-clust-2.Robj")
load("umap-by-cluster-fin-M1-clust-2-3.Robj")
...
save(fin.clust.active.ident, file = "cluster-per-cell-fin-M1-clust-2-3.Robj")
...
save(clust.umap, file = "umap-by-cluster-fin-M1-clust-2-3.Robj")
...
labs(title = "M1, Nipbl Flox/+", x = "UMAP 1", y = "UMAP 2", color = "Cluster") +
...
ggsave(plot = cluster.umap, filename = "umap-by-cluster-fin-M1-clust-2-3.png", device = "png", width = 6.5, height = 4.5, units = "in")

marker.genes <- FindAllMarkers(fin.clust, assay = "SCT_M1", slot = "data", only.pos = TRUE, test.use = "roc", min.pct = 0.2, min.diff.pct = -Inf,  logfc.threshold = 0, return.thresh = 0.5, verbose = TRUE)

save(marker.genes, file = "marker-genes-fin-M1-clust-2-3.Robj")

write.csv(x = marker.genes, file = "marker-genes-fin-M1-clust-2-3.csv")

marker.genes <- marker.genes %>% group_by(cluster) %>% slice_max(myAUC, n = 71/4)

heatmap <- DoHeatmap(fin.clust, assay = "SCT_M1", slot = "scale.data", features = as.vector(marker.genes$gene), group.by = "ident", group.bar = TRUE, label = TRUE, size = 2.3, angle = 0, hjust = 0.5, draw.lines = TRUE) +
scale_fill_gradientn(colors = rev(brewer.pal(n=5, name = "RdBu")), na.value = "white") +
scale_x_discrete(position = "top") +
labs(title = "M1, Nipbl Flox/+", fill = "Norm\nUMI") +
guides(color = FALSE) +
...
ggsave(plot = heatmap, filename = "heatmap-marker-genes-fin-M1-clust-2-3.png", device = "png", units = "in", width = 6.5, height = 9)
