...
ident.clust <- c("W2.1.2")
optim.clust <- list("W2.1.2" = c(2))
...
sub.subclust.umap <- data.frame(flox.sub.subclust@reductions$umap@cell.embeddings, flox.sub.subclust$embryo.id, flox.sub.subclust$wildtype.sub.subclust)
colnames(sub.subclust.umap) <- c("umap.1", "umap.2", "embryo.id", "wildtype.sub.subclust")

filename.robj <- paste0("umap-by-cluster-flox-", n, "-clust-", optim.clust[[i]][t], ".Robj")
save(sub.subclust.umap, file = filename.robj)

umap.label <- sub.subclust.umap %>% group_by(wildtype.sub.subclust) %>% summarize(umap.1 = median(umap.1), umap.2 = median(umap.2))

cluster.umap <- ggplot() +
geom_point(data = sample(sub.subclust.umap), mapping = aes(x = umap.1, y = umap.2, color = wildtype.sub.subclust), size = 0.1) +
geom_text(data = umap.label, mapping = aes(x = umap.1, y = umap.2, label = wildtype.sub.subclust), size = 3.5) +
...
marker.genes <- FindAllMarkers(flox.sub.subclust, assay = name.assay, slot = "data", only.pos = TRUE, test.use = "roc", min.pct = 0.2, min.diff.pct = -Inf,  logfc.threshold = 0, return.thresh = 0.5, verbose = TRUE)
