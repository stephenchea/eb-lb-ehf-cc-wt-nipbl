...
load("/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/cluster-per-cell-fin-integr-clust-6.Robj")

fin.integr$mutant.whole <- fin.integr.active.ident

Idents(fin.integr) <- fin.integr.active.ident

#subset first seven clusters for subclustering
############################################
for (i in levels(fin.integr@active.ident)[c(1, 2, 4)]) {
...
fin.clust <- SCTransform(object = fin.clust, assay = "RNA", variable.features.n = NULL, variable.features.rv.th = 1.1, return.only.var.genes = FALSE, min_cells = 3, n_cells = round(dim(fin.clust)[2]*.5), n_genes = round(dim(fin.clust)[1]*.1), new.assay.name = assay.name, verbose = TRUE)

print("splitting by embryo")
fin.embryo <- SplitObject(fin.clust, split.by = "embryo.id")
...
fin.clust <- RunPCA(fin.clust, assay = "integrated", features = feat.integr, npcs = 50, verbose = TRUE)
...
fin.clust <- RunUMAP(fin.clust, reduction = "pca", dims = 1:pc.number)
filename.robj <- paste0("fin-", i, "-umap.Robj")

clust.umap <- data.frame(fin.clust@reductions$umap@cell.embeddings, fin.clust$embryo.id)
colnames(clust.umap) <- c("umap.1", "umap.2", "embryo.id")
filename.robj <- paste0("umap-by-embryo-fin-", i, "-umap.Robj")
save(clust.umap, file = filename.robj)

umap.embryo <- ggplot() +
geom_point(data = sample(clust.umap), mapping = aes(x = umap.1, y = umap.2, color = embryo.id), size = 0.1) +
...
direct.path <- paste0("/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/", direct.name)
setwd(direct.path)
...
fin.clust <- FindClusters(fin.clust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
...
while (max(as.numeric(fin.clust@active.ident[])) != cluster.number & (loop.count < 1000))
{
    ...
    if(max(as.numeric(fin.clust@active.ident[])) < cluster.number)
    {
        res = res + step
        fin.clust <- FindClusters(object = fin.clust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
        loop.vector <- c(loop.vector, max(as.numeric(fin.clust@active.ident[])))
    }
...
fin.clust.active.ident <- fin.clust@active.ident
filename.robj <- paste0("cluster-per-cell-fin-", i, "-clust-", t, ".Robj")
...
umap.label <- clust.umap %>% group_by(cluster) %>% summarize(umap.1 = median(umap.1), umap.2 = median(umap.2))

cluster.umap <- ggplot() +
geom_point(data = sample(clust.umap), mapping = aes(x = umap.1, y = umap.2, color = cluster), size = 0.1) +
...
filename.png <- paste0("umap-by-cluster-fin-", i, "-clust-", t, ".png")
