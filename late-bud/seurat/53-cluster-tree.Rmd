...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/fin-M1-clust-2/fin-M*-cluster-iters /Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M1-clust-2
```

Set cluster identities and max cluster iterations.
```{r}
ident.clust <- c("M1_1", "M1_2")
max.iters <- c(10, 10)
```
...
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M1-clust-2/fin-", ident.clust[e], "-cluster-iters/cluster-per-cell-fin-", ident.clust[e], "-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(fin.subclust.active.ident)
}
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = paste0(str_replace(ident.clust[e], "_", "."), ", Nipbl FIN/+"), x = "No. of Clusters", y = "Total Shannon Entropy") +
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M1-clust-2/summary-shannon-fin-", ident.clust[e], ".png")
...
cluster.tree <- ggraph(cluster.tree.layout, layout = "tree") +
  ...
  labs(title = paste0(str_replace(ident.clust[e], "_", "."), ", Nipbl FIN/+"), y = "No. of Clusters", size = "No. of Cells\nin Cluster", color = "Shannon\nEntropy", edge_width = "No. of Cells\nContributing to\nProceeding\nCluster", edge_alpha = "Fraction of\nCells Within\nProceeding\nCluster") +
  theme_classic(base_size =7) +
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M1-clust-2/cluster-tree-fin-", ident.clust[e], ".png")
...
Download cluster iterations.
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M*-cluster-iters /Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3
```

Set cluster identities and max cluster iterations.
```{r}
ident.clust <- c("M2_1", "M2_2")
max.iters <- c(10, 10)
```
...
print("open cluster-per-cell files and store them in list above")
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-", ident.clust[e], "-cluster-iters/cluster-per-cell-fin-", ident.clust[e], "-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(fin.subclust.active.ident)
}
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/summary-shannon-fin-", ident.clust[e], ".png")
ggsave(plot = shannon.plot, filename = filename.png, device = "png", units = "in", width = 6.5, height = 4.5)
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/cluster-tree-fin-", ident.clust[e], ".png")
ggsave(plot = cluster.tree, filename = filename.png, device = "png", units = "in", width = 6.5, height = 9)
