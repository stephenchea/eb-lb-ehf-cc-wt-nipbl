...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/all-aggr-cluster-iters /Volumes/Bud_SSD/20210505-seurat
```
...
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/all-aggr-cluster-iters/cluster-per-cell-all-aggr-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(all.aggr.active.ident)
}
...
write.csv(tidy.edge, file = "edges-flox-mes-clustered-iterations.csv")
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = "Nipbl Flox/+ & FIN/+", x = "No. of Clusters", y = "Total Shannon Entropy", color = "") +
  theme_classic(base_size = 7) + 
  theme(plot.title = element_text(hjust = 0.5), legend.position = "right")

shannon.plot

ggsave(plot = shannon.plot, filename = "/Volumes/Bud_SSD/20210505-seurat/summary-shannon-all-aggr.png", device = "png", units = "in", width = 6.5, height = 4.5)
```
...
cluster.tree <- ggraph(graph = cluster.tree.layout, layout = "tree") +
 ...
 scale_edge_alpha_continuous(range = c(0.2, 1)) +
  ...
  annotate("rect", xmin=-Inf, xmax=Inf, ymin=9.5, ymax=10.5, color="red3", alpha = 0)

cluster.tree

ggsave(plot = cluster.tree, filename = "/Volumes/Bud_SSD/20210505-seurat/cluster-tree-all-aggr.png", device = "png", units = "in", width = 6.5, height = 9)
```
Looks like 5 clusters is good. 
