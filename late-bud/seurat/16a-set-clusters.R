...
load("flox-integr-cluster-iters/cluster-per-cell-flox-integr-clust-5.Robj")

flox.integr@active.ident <- flox.integr.active.ident
flox.integr <- RenameIdents(object = flox.integr, "1" = "W1", "2" = "W2", "3" = "W3", "4" = "W4", "5" = "W5")
flox.integr@active.ident <- factor(flox.integr@active.ident, levels = c("W1", "W2", "W3", "W4", "W5"))
flox.integr$wildtype.whole <- flox.integr@active.ident

dir.create("flox-integr-clust-5")
setwd("/dfs6/pub/schea2/20210505-seurat/flox-integr-clust-5")

#save(flox.integr, file = "flox-integr-clust-5.Robj")

flox.integr.active.ident <- flox.integr$wildtype.whole
save(flox.integr.active.ident, file = "cluster-per-cell-flox-integr-clust-5.Robj")
...
save(whole.umap, file = "umap-by-cluster-flox-integr-clust-5.Robj")
...
ggsave(plot = cluster.umap, filename = "umap-by-cluster-flox-integr-clust-5.png", device = "png", width = 6.5, height = 4.5, units = "in")

marker.genes <- FindAllMarkers(flox.integr, assay = "SCT_GENOTYPE", slot = "data", only.pos = TRUE, test.use = "roc", min.pct = 0.2, min.diff.pct = -Inf,  logfc.threshold = 0, return.thresh = 0.5, verbose = TRUE)

save(marker.genes, file = "marker-genes-flox-integr-clust-5.Robj")

write.csv(x = marker.genes, file = "marker-genes-flox-integr-clust-5.csv")
...
labs(title = "Whole, Nipbl Flox/+", x = "Cluster", y = "Top Genes by AUC", fill = "Centered\nNorm UMI") +
guides(color = FALSE) +
