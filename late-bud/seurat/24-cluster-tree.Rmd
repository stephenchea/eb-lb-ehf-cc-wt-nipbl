...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2/flox-W*-cluster-iters /Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2
```

Set cluster identities and max cluster iterations.
```{r}
ident.clust <- c("W2_1_1", "W2_1_2")
max.iters <- c(5, 5)
```
...
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2/flox-", ident.clust[e], "-cluster-iters/cluster-per-cell-flox-", ident.clust[e], "-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(flox.sub.subclust.active.ident)
}
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = paste0(str_replace_all(ident.clust[e], "_", "."), ", Nipbl Flox/+"), x = "No. of Clusters", y = "Total Shannon Entropy") +
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2/summary-shannon-flox-", ident.clust[e], ".png")
...
cluster.tree <- ggraph(cluster.tree.layout, layout = "tree") +
  ...
  labs(title = paste0(str_replace_all(ident.clust[e], "_", "."), ", Nipbl Flox/+"), y = "No. of Clusters", size = "No. of Cells\nin Cluster", color = "Shannon\nEntropy", edge_width = "No. of Cells\nContributing to\nProceeding\nCluster", edge_alpha = "Fraction of\nCells Within\nProceeding\nCluster") +
  theme_classic(base_size =7) +
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2/cluster-tree-flox-", ident.clust[e], ".png")
