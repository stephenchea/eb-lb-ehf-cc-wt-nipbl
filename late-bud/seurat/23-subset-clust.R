...
load("flox-W2_1-neighbors.Robj")

setwd("/dfs6/pub/schea2/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2")
load("cluster-per-cell-flox-W2_1-clust-2.Robj")

flox.subclust$wildtype.subclust <- flox.subclust.active.ident
Idents(flox.subclust) <- flox.subclust.active.ident

save(flox.subclust, file = "flox-W2_1-clust-2.Robj")

for (i in c("W2.1.1", "W2.1.2")) {
        
    print(i)
    print("subsetting cluster")
    flox.sub.subclust <- subset(flox.subclust, idents = i)
    
    n <- str_replace_all(i,"[.]","_")
    assay.name <- paste0("SCT_", n)

    print("applying sc transform to entire cluster")
    flox.sub.subclust <- SCTransform(object = flox.sub.subclust, assay = "RNA", variable.features.n = NULL, variable.features.rv.th = 1.1, return.only.var.genes = FALSE, min_cells = 3, n_cells = round(dim(flox.sub.subclust)[2]*.5), n_genes = round(dim(flox.sub.subclust)[1]*.1), new.assay.name = assay.name, verbose = TRUE)

    print("splitting by embryo")
    flox.embryo <- SplitObject(flox.sub.subclust, split.by = "embryo.id")
    ...
    flox.sub.subclust <- RunPCA(flox.sub.subclust, assay = "integrated", features = feat.integr, npcs = 50, verbose = TRUE)
    ...
    flox.sub.subclust <- RunUMAP(flox.sub.subclust, reduction = "pca", dims = 1:pc.number, return.model = TRUE)
    filename.robj <- paste0("flox-", n, "-umap.Robj")

    sub.subclust.umap <- data.frame(flox.sub.subclust@reductions$umap@cell.embeddings, flox.sub.subclust$embryo.id)
    colnames(sub.subclust.umap) <- c("umap.1", "umap.2", "embryo.id")
    filename.robj <- paste0("umap-by-embryo-flox-", n, "-umap.Robj")
    save(sub.subclust.umap, file = filename.robj)

    umap.embryo <- ggplot() +
    geom_point(data = sample(sub.subclust.umap), mapping = aes(x = umap.1, y = umap.2, color = embryo.id), size = 0.1) +
    ...
    flox.sub.subclust <- FindNeighbors(flox.sub.subclust, reduction = "pca", dims = 1:pc.number, graph.name = c(name.graph, name.graph))

    filename.robj <- paste0("flox-", n, "-neighbors.Robj")
    save(flox.sub.subclust, file = filename.robj)
    ...
    direct.path <- paste0("/dfs6/pub/schea2/20210505-seurat/flox-integr-clust-5/flox-W2-clust-2/flox-W2_1-clust-2/", direct.name)
    setwd(direct.path)
    ...
    flox.sub.subclust <- FindClusters(flox.sub.subclust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
    ...
    while (max(as.numeric(flox.sub.subclust@active.ident[])) != cluster.number & (loop.count < 1000))
    {
        ...
        if(max(as.numeric(flox.sub.subclust@active.ident[])) < cluster.number)
        {
            res = res + step
            flox.sub.subclust <- FindClusters(object = flox.sub.subclust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
            loop.vector <- c(loop.vector, max(as.numeric(flox.sub.subclust@active.ident[])))
        }
    ...
    print(res)
    print(length(levels(flox.sub.subclust)))

    print("saving cluster identities")
    renumbered.cluster.ids <- 1:t
    names(renumbered.cluster.ids) <- levels(flox.sub.subclust)
    flox.sub.subclust <- RenameIdents(flox.sub.subclust, renumbered.cluster.ids)
    flox.sub.subclust.active.ident <- flox.sub.subclust@active.ident
    ...
    sub.subclust.umap <- data.frame(flox.sub.subclust@reductions$umap@cell.embeddings, flox.sub.subclust@active.ident)
    colnames(sub.subclust.umap) <- c("umap.1", "umap.2", "cluster")

    umap.label <- sub.subclust.umap %>% group_by(cluster) %>% summarize(umap.1 = median(umap.1), umap.2 = median(umap.2))

    cluster.umap <- ggplot() +
    geom_point(data = sample(sub.subclust.umap), mapping = aes(x = umap.1, y = umap.2, color = cluster), size = 0.1) +
