...
Load Flox cells.
```{r}
load("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-MES-clust-11/flox-MES-umap.Robj")
```
...
Load markers.
```{r}
load("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-MES-clust-11/marker-genes-flox-MES-clust-11.Robj")
```
...
```{r}
marker.genes %>% group_by(cluster) %>% slice_max(myAUC, n = 71/(11/2))
target.genes <- marker.genes %>% group_by(cluster) %>% slice_max(myAUC, n = 71/(11/2)) %>% pull(gene)
```
...
target.clusters <- marker.genes %>% group_by(cluster) %>% slice_max(myAUC, n = 71/(11/2)) %>% pull(cluster)
names(target.clusters) <- colnames(expr.umap)[1:length(target.genes)]
```

```{r}
dir.create("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-MES-clust-11/feat-plot-top-markers")
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/flox-integr-clust-5/flox-MES-clust-11/feat-plot-top-markers/", target.clusters[i], "-", string.loop.count, "-", target.genes[i], "-feat-plot-flox-mes.png")
