...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2/fin-M*-cluster-iters /Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2
```

Set cluster identities and max cluster iterations.
```{r}
ident.clust <- c("M2_1_1")
max.iters <- c(5)
```
...
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2/fin-", ident.clust[e], "-cluster-iters/cluster-per-cell-fin-", ident.clust[e], "-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(fin.sub.subclust.active.ident)
}
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = paste0(str_replace_all(ident.clust[e], "_", "."), ", Nipbl FIN/+"), x = "No. of Clusters", y = "Total Shannon Entropy") +
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2/summary-shannon-fin-", ident.clust[e], ".png")
...
cluster.tree <- ggraph(cluster.tree.layout, layout = "tree") +
  ...
  labs(title = paste0(str_replace_all(ident.clust[e], "_", "."), ", Nipbl FIN/+"), y = "No. of Clusters", size = "No. of Cells\nin Cluster", color = "Shannon\nEntropy", edge_width = "No. of Cells\nContributing to\nProceeding\nCluster", edge_alpha = "Fraction of\nCells Within\nProceeding\nCluster") +
  theme_classic(base_size =7) +
...
filename.png <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2/cluster-tree-fin-", ident.clust[e], ".png")
