...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/fin-integr-cluster-iters /Volumes/Bud_SSD/20210505-seurat
```
...
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/fin-integr-cluster-iters/cluster-per-cell-fin-integr-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(fin.integr.active.ident)
}
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = "Whole, Nipbl FIN/+", x = "No. of Clusters", y = "Total Shannon Entropy", color = "") +
  theme_classic(base_size = 7) + 
  theme(plot.title = element_text(hjust = 0.5), legend.position = "right")

shannon.plot

ggsave(plot = shannon.plot, filename = "/Volumes/Bud_SSD/20210505-seurat/summary-shannon-fin-integr.png", device = "png", units = "in", width = 6.5, height = 4.5)
```
...
cluster.tree <- ggraph(cluster.tree.layout, layout = "tree") +
  ...
  annotate("rect", xmin=-Inf, xmax=Inf, ymin=8.5, ymax=9.5, color="red3", alpha = 0)

cluster.tree

ggsave(plot = cluster.tree, filename = "/Volumes/Bud_SSD/20210505-seurat/cluster-tree-fin-integr.png", device = "png", units = "in", width = 6.5, height = 9)
