...
for (i in c("M1.1", "M1.2")) {
    ...
    direct.path <- paste0("/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/fin-M1-clust-2/", direct.name)
    setwd(direct.path)
    ...
    fin.subclust <- FindClusters(fin.subclust, resolution = res, graph.name = name.graph, random.seed = 12, verbose = TRUE)
    ...
    while (max(as.numeric(fin.subclust@active.ident[])) != cluster.number & (loop.count < 1000))
    {
        ...
        {
            res = res + step
            fin.subclust <- FindClusters(object = fin.subclust, resolution = res, graph.name = name.graph, random.seed = 12, verbose = TRUE)
