...
load("/dfs6/pub/schea2/20210505-seurat/flox-integr-clust-5/cluster-per-cell-flox-integr-clust-5.Robj")
...
############################################
for (i in levels(flox.integr@active.ident)[1:4]) {
...
flox.clust <- SCTransform(object = flox.clust, assay = "RNA", variable.features.n = NULL, variable.features.rv.th = 1.1, return.only.var.genes = FALSE, min_cells = 3, n_cells = round(dim(flox.clust)[2]*.5), n_genes = round(dim(flox.clust)[1]*.1), new.assay.name = assay.name, verbose = TRUE)

print("splitting by embryo")
flox.embryo <- SplitObject(flox.clust, split.by = "embryo.id")
...
flox.clust <- RunPCA(flox.clust, assay = "integrated", features = feat.integr, npcs = 50, verbose = TRUE)
...
clust.umap <- data.frame(flox.clust@reductions$umap@cell.embeddings, flox.clust$embryo.id)
colnames(clust.umap) <- c("umap.1", "umap.2", "embryo.id")
filename.robj <- paste0("umap-by-embryo-flox-", i, "-umap.Robj")
...
flox.clust <- FindClusters(flox.clust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
...
while (max(as.numeric(flox.clust@active.ident[])) != cluster.number & (loop.count < 1000))
{
    ...
    if(max(as.numeric(flox.clust@active.ident[])) < cluster.number)
    {
        res = res + step
        flox.clust <- FindClusters(object = flox.clust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
        loop.vector <- c(loop.vector, max(as.numeric(flox.clust@active.ident[])))
    }
...
print(res)
print(length(levels(flox.clust)))
...
flox.clust.active.ident <- flox.clust@active.ident
filename.robj <- paste0("cluster-per-cell-flox-", i, "-clust-", t, ".Robj")
...
filename.png <- paste0("umap-by-cluster-flox-", i, "-clust-", t, ".png")
