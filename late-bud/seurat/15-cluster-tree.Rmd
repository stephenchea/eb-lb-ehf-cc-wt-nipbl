...
```{bash}
scp -r schea2@hpc3.rcic.uci.edu:/dfs6/pub/schea2/20210505-seurat/flox-integr-cluster-iters /Volumes/Bud_SSD/20210505-seurat
```
...
for (i in 2:max.res) {
  file.name <- paste0("/Volumes/Bud_SSD/20210505-seurat/flox-integr-cluster-iters/cluster-per-cell-flox-integr-clust-", i, ".Robj")
  load(file.name)
  cluster.cell.list[[i-1]] <- as.numeric(flox.integr.active.ident)
}
...
shannon.plot <- ggplot(data = tidy.node, aes(x = resolution, y = shannon, group = 1)) + 
  ...
  labs(title = "Whole, Nipbl Flox/+", x = "No. of Clusters", y = "Total Shannon Entropy", color = "") +
  theme_classic(base_size = 7) + 
  theme(plot.title = element_text(hjust = 0.5), legend.position = "right")

shannon.plot

ggsave(plot = shannon.plot, filename = "/Volumes/Bud_SSD/20210505-seurat/summary-shannon-flox-integr.png", device = "png", units = "in", width = 6.5, height = 4.5)
```
...
ggsave(plot = cluster.tree, filename = "/Volumes/Bud_SSD/20210505-seurat/cluster-tree-flox-integr.png", device = "png", units = "in", width = 6.5, height = 9)


```
Looks like 9 clusters is good. 
