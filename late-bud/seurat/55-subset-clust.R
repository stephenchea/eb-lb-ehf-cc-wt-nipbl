...
load("fin-M2_1-neighbors.Robj")

setwd("/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2")
load("cluster-per-cell-fin-M2_1-clust-2.Robj")

fin.subclust$mutant.subclust <- fin.subclust.active.ident
Idents(fin.subclust) <- fin.subclust.active.ident

save(fin.subclust, file = "fin-M2_1-clust-2.Robj")

for (i in c("M2.1.1")) {
        
    print(i)
    print("subsetting cluster")
    fin.sub.subclust <- subset(fin.subclust, idents = i)
    
    n <- str_replace_all(i,"[.]","_")
    assay.name <- paste0("SCT_", n)

    print("applying sc transform to entire cluster")
    fin.sub.subclust <- SCTransform(object = fin.sub.subclust, assay = "RNA", variable.features.n = NULL, variable.features.rv.th = 1.1, return.only.var.genes = FALSE, min_cells = 3, n_cells = round(dim(fin.sub.subclust)[2]*.5), n_genes = round(dim(fin.sub.subclust)[1]*.1), new.assay.name = assay.name, verbose = TRUE)

    print("splitting by embryo")
    fin.embryo <- SplitObject(fin.sub.subclust, split.by = "embryo.id")
    ...
    fin.sub.subclust <- RunPCA(fin.sub.subclust, assay = "integrated", features = feat.integr, npcs = 50, verbose = TRUE)
    ...
    fin.sub.subclust <- RunUMAP(fin.sub.subclust, reduction = "pca", dims = 1:pc.number)
    filename.robj <- paste0("fin-", n, "-umap.Robj")

    sub.subclust.umap <- data.frame(fin.sub.subclust@reductions$umap@cell.embeddings, fin.sub.subclust$embryo.id)
    ...
    fin.sub.subclust <- FindNeighbors(fin.sub.subclust, reduction = "pca", dims = 1:pc.number, graph.name = c(name.graph, name.graph))
    ...
    direct.path <- paste0("/dfs6/pub/schea2/20210505-seurat/fin-integr-clust-6/fin-M2-clust-3/fin-M2_1-clust-2/", direct.name)
    setwd(direct.path)
    ...
    fin.sub.subclust <- FindClusters(fin.sub.subclust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
    ...
    while (max(as.numeric(fin.sub.subclust@active.ident[])) != cluster.number & (loop.count < 1000))
    {
        ...
        if(max(as.numeric(fin.sub.subclust@active.ident[])) < cluster.number)
        {
            res = res + step
            fin.sub.subclust <- FindClusters(object = fin.sub.subclust, resolution = res, graph.name = name.graph, random.seed = 1, verbose = TRUE)
            loop.vector <- c(loop.vector, max(as.numeric(fin.sub.subclust@active.ident[])))
        }
    ...
    print(res)
    print(length(levels(fin.sub.subclust)))

    print("saving cluster identities")
    renumbered.cluster.ids <- 1:t
    names(renumbered.cluster.ids) <- levels(fin.sub.subclust)
    fin.sub.subclust <- RenameIdents(fin.sub.subclust, renumbered.cluster.ids)
    fin.sub.subclust.active.ident <- fin.sub.subclust@active.ident
    ...
    sub.subclust.umap <- data.frame(fin.sub.subclust@reductions$umap@cell.embeddings, fin.sub.subclust@active.ident)
